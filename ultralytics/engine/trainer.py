...
from ultralytics.nn.modules import Detect, Attention, RepNCSPELAN4
...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        if self.prune:
            ...
            pruned_macs, pruned_nparams = tp.utils.count_ops_and_params(
                self.model, example_inputs.to(self.device)
            )
            print(
                f"After pruning iter : MACs={pruned_macs / 1e9} G, #Params={pruned_nparams / 1e6} M, "
            )
        ...
        # FORCE UNFREEZE: bỏ trạng thái requires_grad=False bị mang theo từ checkpoint
        ...
        if RANK > -1 and world_size > 1:  # DDP
            dist.broadcast(
                self.amp.int(), src=0
            )  # broadcast from rank 0 to all other ranks; gloo errors with boolean
    ...
    def get_dataset(self):
        ...
        try:
            ...
            elif self.args.data.rsplit(".", 1)[-1] in {
                "yaml",
                "yml",
    ...
    def setup_model(self):
        ...
        if str(self.model).endswith(".pt"):
            ...
            # ✅ CỰC QUAN TRỌNG: resume thì dùng thẳng model trong checkpoint (đã prune)
            if self.args.resume:
                self.model = weights
                return ckpt
