...
class BboxLoss(nn.Module):
    ...
    def __init__(
        self, reg_max: int = 16, bbox_loss_type: str = "ciou", alpha_iou: float = 2.0
    ):
        ...
        self.bbox_loss_type = (bbox_loss_type or "ciou").lower()
    ...
    def forward(
        ...
        # IoU loss: type selected by self.bbox_loss_type
        if self.bbox_loss_type == "siou":
            iou = bbox_iou(
                pred_bboxes[fg_mask],
        ...
        elif self.bbox_loss_type == "alpha_iou":
            iou = bbox_iou(
                pred_bboxes[fg_mask],
                target_bboxes[fg_mask],
                xywh=False,
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10):  # model must be de-paralleled
        ...
        # Prefer values set on the model (train.py may set model.bbox_loss_type / model.alpha_iou),
        # then fall back to model.args/h.
        if isinstance(h, dict):
            bbox_loss_type = getattr(
                model, "bbox_loss_type", h.get("bbox_loss_type", "ciou")
            )
            alpha_iou = getattr(model, "alpha_iou", h.get("alpha_iou", 2.0))
        else:
            bbox_loss_type = getattr(
                model, "bbox_loss_type", getattr(h, "bbox_loss_type", "ciou")
            )
            alpha_iou = getattr(model, "alpha_iou", getattr(h, "alpha_iou", 2.0))

        bbox_loss_type = (bbox_loss_type or "ciou").lower()
    ...
    def bbox_decode(
        ...
        if self.use_dfl:
            ...
            # ✅ đảm bảo proj cùng device + dtype với pred_dist
