...
def compute_calm_row_loss(
    ...
    *,
    ...
    # --- LSL line-stability, per image (tempered precision weights, fit per image below) ---
    terms = []
    for bi in range(B):
        mb = fg_mask[bi]
        if int(mb.sum()) < 3:
        ...
        cxp, cyp = cx_pred[bi][mb], cy_pred[bi][mb]
        wp = precision_weights(sig_cx2[bi][mb])   # tempered per-image, matches eval readout
        # robust (Huber-IRLS) precision-weighted fit, matching the eval-time readout
        a_p, b_p, _ = gls_line_fit(cxp, cyp, wp, with_cov=False, irls_iters=3)
