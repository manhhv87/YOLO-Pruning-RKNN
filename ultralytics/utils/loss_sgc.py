...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_type: str = "giou", alpha_iou: float = 2.0):
        """Initialize the BboxLoss module with regularization maximum and DFL settings."""
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None

        # IoU type: "iou", "giou", "diou", "ciou", "eiou", "siou", "alpha_iou"        
        self.iou_type = (iou_type or "giou").lower()
        self.alpha_iou = alpha_iou

        # ----- v2 tuned -----
        self.use_center = True
        self.center_lambda = 0.05
        self.center_max_rel_error = 2.0

        self.use_thickness = True
        self.thickness_lambda = 0.02
        self.thickness_max_rel_error = 2.0

        # warmup + IoU gating
        self.loss_warmup_iters = 2000
        self._loss_iter = 0
        self.iou_gate_thr = 0.30
        self.iou_gate_k = 10.0

        # center normalize: "short" (khuyến nghị) hoặc "width" (đúng theo doc)
        self.center_norm = "short"

        self.eps = 1e-6        
        # -----------------------------------------------------------

    def _centerline_loss(self, pred_bboxes, target_bboxes, sample_weight, target_scores_sum, fg_mask):
        if not fg_mask.any():
            return torch.tensor(0.0, device=pred_bboxes.device)

        pb = pred_bboxes[fg_mask]
        tb = target_bboxes[fg_mask]
        w  = sample_weight[fg_mask]

        tcx = (tb[:, 0] + tb[:, 2]) * 0.5
        pcx = (pb[:, 0] + pb[:, 2]) * 0.5

        t_wh = (tb[:, 2:4] - tb[:, 0:2]).clamp(min=self.eps)
        t_w = t_wh[:, 0]
        t_short = t_wh.min(dim=1).values

        denom = t_short if self.center_norm == "short" else t_w
        denom = denom.clamp(min=self.eps)

        rel = (pcx - tcx) / denom
        rel = rel.clamp(min=-self.center_max_rel_error, max=self.center_max_rel_error)

        per = F.smooth_l1_loss(rel, torch.zeros_like(rel), reduction="none")
        return (per * w).sum() / (target_scores_sum + self.eps)

    def _thickness_loss(self, pred_bboxes, target_bboxes, sample_weight, target_scores_sum, fg_mask):
        if not fg_mask.any():
            return torch.tensor(0.0, device=pred_bboxes.device)
        ...
        t_wh = (tb[:, 2:4] - tb[:, 0:2]).clamp(min=self.eps)
        p_wh = (pb[:, 2:4] - pb[:, 0:2]).clamp(min=self.eps)

        t_short = t_wh.min(dim=1).values.clamp(min=self.eps)
        p_short = p_wh.min(dim=1).values.clamp(min=self.eps)

        # clip theo log để đối xứng (ratio)
        max_log = math.log(1.0 + self.thickness_max_rel_error)  # e.g. 2.0 -> ln(3)
        log_err = torch.log(p_short / t_short).clamp(min=-max_log, max=max_log)

        per = F.smooth_l1_loss(log_err, torch.zeros_like(log_err), reduction="none")
        return (per * w).sum() / (target_scores_sum + self.eps)
    ...
    def forward(
        ...
        # --- v2: warmup + iou-gate ---
        self._loss_iter += 1
        warm = min(1.0, float(self._loss_iter) / float(self.loss_warmup_iters))

        iou_pos = iou.detach().view(-1)
        gate = torch.sigmoid((iou_pos - self.iou_gate_thr) * self.iou_gate_k)  # (N_pos,)

        sample_weight = base_weight.clone()
        sample_weight[fg_mask] = sample_weight[fg_mask] * gate

        if self.use_center and fg_mask.any():
            loss_center = self._centerline_loss(pred_bboxes, target_bboxes, sample_weight, target_scores_sum, fg_mask)
            loss_iou = loss_iou + (warm * self.center_lambda) * loss_center

        if self.use_thickness and fg_mask.any():
            loss_thick = self._thickness_loss(pred_bboxes, target_bboxes, sample_weight, target_scores_sum, fg_mask)
            loss_iou = loss_iou + (warm * self.thickness_lambda) * loss_thick
